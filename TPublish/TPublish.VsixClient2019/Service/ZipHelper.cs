...
namespace TPublish.VsixClient2019.Service
{
    public class ZipHelper
    {
        ...
        /// <param name="baseDir"></param>
        /// <returns></returns>
        public static bool ZipManyFilesOrDictorys(IList<string> folderOrFileList, string zipedFile, string baseDir = "")
        {
        ...
        private static void DoZip(string fileOrDir, string baseDir, ZipOutputStream s)
        {
            string parentPath = "";
            if (fileOrDir.StartsWith(baseDir))
            {
            ...
            if (Directory.Exists(fileOrDir))
            {
                ZipFileDictory(fileOrDir, s, parentPath);
            }
            else
            {
                //文件
                ZipFileWithStream(fileOrDir, s, parentPath);
            }
        }
        ...
        private static void AddEmptyDir(string folderToZip, ZipOutputStream s, string parentFolderName)
        {
            try
            {
                if (emptyDir.Contains(folderToZip))
                {
                    return;
                }
